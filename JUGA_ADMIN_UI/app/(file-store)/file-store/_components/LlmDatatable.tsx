...
type LLMTableRow = FileStore | { id: "new" | "update"; identifier?: string; uri?: string };
...
function LLMEmbeddingDatatable() {
  ...
  const validateLLMData = useCallback((data: any): string => {
    if (!data.identifier.trim()) return "İdentifier zorunludur";
    return "";
  }, []);
  ...
  const handleSaveNewLLM = useCallback(() => {
    ...
    setValidationError("");
    createLLM.mutate(newLLMDataRef.current as any, {
      onSuccess: (response) => {
  ...
  const handleSaveEditLLM = useCallback(() => {
    ...
    setValidationError("");
    updateLLM.mutate(editingLLMDataRef.current as any, {
      onSuccess: () => {
  ...
  const columns = useMemo<ColumnDef<LLMTableRow>[]>(
    () => [
          ...
          if (row.original.id === "new") {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="e.g., gpt-4-turbo" defaultValue={newLLMDataRef.current.id} onChange={(value) => handleNewLLMChange("id", value)} className="max-w-xs" />
              </div>
            );
          }
      ...
      },
      {
        ...
        header: "identifier",
        cell: ({ row }) => {
          if (row.original.identifier === "new") {
            return (
              <div className="flex items-center">
                <div className="w-10 h-10 bg-primary rounded-full flex items-center justify-center mr-4 shrink-0">
                  <Bot className="w-5 h-5 text-white" />
                </div>
                <EditableCell placeholder="Provider ID" defaultValue={String(newLLMDataRef.current.identifier || "")} onChange={(value) => handleNewLLMChange("identifier", value)} className="max-w-xs" />
              </div>
            );
          }
          ...
          if (editingRowId === llm.id) {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="Provider ID" defaultValue={String(editingLLMDataRef.current?.identifier || "")} onChange={(value) => handleEditingLLMChange("identifier", value)} className="max-w-xs" />
              </div>
            );
          }
          ...
          return (
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <div className="text-sm font-medium text-card-foreground">{llm.identifier}</div>
              </div>
            </div>
          );
        },
      },
      {
        ...
        cell: ({ row }) => {
          const llm = row.original as FileStore;
          if (row.original.uri === "new") {
            return (
              <div className="flex items-center">
                <div className="w-10 h-10 bg-primary rounded-full flex items-center justify-center mr-4 shrink-0">
                  <Bot className="w-5 h-5 text-white" />
                </div>
                <EditableCell placeholder="e.g., 4096" defaultValue={String(newLLMDataRef.current.uri)} onChange={(value) => handleNewLLMChange("uri", value)} className="max-w-xs" />
              </div>
            );
          }
          ...
          if (editingRowId === llm.id) {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="e.g., 4096" defaultValue={String(editingLLMDataRef.current?.uri || 0)} onChange={(value) => handleEditingLLMChange("uri", value)} className="max-w-xs" />
              </div>
            );
          }
          return (
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <div className="text-sm font-medium text-card-foreground">{llm.uri}</div>
              </div>
            </div>
          );
        },
