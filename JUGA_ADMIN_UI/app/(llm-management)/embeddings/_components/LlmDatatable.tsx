...
import { LLM_EMBEDDING_CONSTANTS } from "../_constants/LlmModelsConstants";
...
type LLMTableRow = EmbeddingModel | (Omit<EmbeddingModel, "llmProviderId"> & { llmProviderId: "new" | "update" });

const INITIAL_NEW_LLM_STATE: Omit<EmbeddingModel, "llmProviderId" | "id"> = {
  url: "",
  modelName: "",
  vectorSize: 0,
  maxInputTokenSize: 0,
...
function LLMEmbeddingDatatable() {
  ...
  const newLLMDataRef = useRef<Omit<EmbeddingModel, "id">>(INITIAL_NEW_LLM_STATE);
  const editingLLMDataRef = useRef<EmbeddingModel | null>(null);
  ...
  const validateLLMData = useCallback((data: Omit<EmbeddingModel, "id">): string => {
  ...
  const handleNewLLMChange = useCallback(
    (field: keyof Omit<EmbeddingModel, "id">, value: string | number) => {
  ...
  const handleEditingLLMChange = useCallback(
    (field: keyof EmbeddingModel, value: string | number) => {
      if (validationError) setValidationError("");
      if (!editingLLMDataRef.current) return;
  ...
  const handleEditLLMModel = useCallback((llm: EmbeddingModel) => {
    setIsAddingNew(false);
    ...
    () => [
      {
        ...
        header: "Model Name",
        cell: ({ row }) => {
          const llm = row.original as EmbeddingModel;
          if (row.original.llmProviderId === "new") {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="e.g., gpt-4-turbo" defaultValue={newLLMDataRef.current.modelName} onChange={(value) => handleNewLLMChange("modelName", value)} className="max-w-xs" />
              </div>
            );
          }
          ...
          if (editingRowId === llm.id) {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="e.g., gpt-4-turbo" defaultValue={editingLLMDataRef.current?.modelName || ""} onChange={(value) => handleEditingLLMChange("modelName", value)} className="max-w-xs" />
              </div>
            );
          }
          ...
          return (
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <div className="text-sm font-medium text-card-foreground">{llm.modelName}</div>
        ...
        cell: ({ row }) => {
          if (row.original.llmProviderId === "new") {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="Provider ID" defaultValue={String(newLLMDataRef.current.llmProviderId || "")} onChange={(value) => handleNewLLMChange("llmProviderId", value)} className="max-w-xs" />
              </div>
            );
          }
          ...
          if (editingRowId === llm.id) {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="Provider ID" defaultValue={String(editingLLMDataRef.current?.llmProviderId || "")} onChange={(value) => handleEditingLLMChange("llmProviderId", value)} className="max-w-xs" />
              </div>
            );
          }
          ...
          return (
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <div className="text-sm font-medium text-card-foreground">{llm.llmProviderId}</div>
              </div>
            </div>
          );
        },
          ...
          if (row.original.llmProviderId === "new") {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="e.g., 4096" defaultValue={String(newLLMDataRef.current.maxInputTokenSize)} onChange={(value) => handleNewLLMChange("maxInputTokenSize", value)} className="max-w-xs" />
              </div>
            );
          }
          ...
          if (editingRowId === llm.id) {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="e.g., 4096" defaultValue={String(editingLLMDataRef.current?.maxInputTokenSize || 0)} onChange={(value) => handleEditingLLMChange("maxInputTokenSize", value)} className="max-w-xs" />
              </div>
            );
          }
          return (
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <div className="text-sm font-medium text-card-foreground">{llm.maxInputTokenSize}</div>
              </div>
            </div>
          );
        },
          ...
          if (row.original.llmProviderId === "new") {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="https://api.example.com" defaultValue={newLLMDataRef.current.url} onChange={(value) => handleNewLLMChange("url", value)} className="max-w-xs" />
              </div>
            );
          }
          if (editingRowId === llm.id) {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="https://api.example.com" defaultValue={editingLLMDataRef.current?.url || ""} onChange={(value) => handleEditingLLMChange("url", value)} className="max-w-xs" />
              </div>
            );
          }
          return (
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <div className="text-sm font-medium text-card-foreground">{llm.url}</div>
              </div>
            </div>
          );
        },
          ...
          if (row.original.llmProviderId === "new") {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="https://api.example.com" defaultValue={newLLMDataRef.current.vectorSize} onChange={(value) => handleNewLLMChange("vectorSize", value)} className="max-w-xs" />
              </div>
            );
          }
          if (editingRowId === llm.id) {
            return (
              <div className="flex items-center">
                ...
                <EditableCell placeholder="https://api.example.com" defaultValue={editingLLMDataRef.current?.vectorSize || 0} onChange={(value) => handleEditingLLMChange("vectorSize", value)} className="max-w-xs" />
              </div>
            );
          }
          return (
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <div className="text-sm font-medium text-card-foreground">{llm.vectorSize}</div>
              </div>
            </div>
          );
        },
  ...
  const tableData = useMemo(() => {
    if (isAddingNew) {
      const newRow: LLMTableRow = { ...INITIAL_NEW_LLM_STATE, llmProviderId: "new" };
  ...
  return (
    <div className="m-4">
      ...
      <DataTable
        ...
        data={tableData}
        columns={columns}
        title={LLM_EMBEDDING_CONSTANTS.title}
        description={LLM_EMBEDDING_CONSTANTS.description}
        actionButton={
