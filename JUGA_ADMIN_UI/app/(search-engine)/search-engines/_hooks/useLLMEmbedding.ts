...
import { createSearchEnginesMutationOptions, deleteSearchEnginesMutationOptions, updateSearchEnginesMutationOptions } from "./MutationOptions";
...
export function useCreateLLMEmbedding() {
  ...
  return useMutation(createSearchEnginesMutationOptions(queryClient));
}
...
export function useUpdateLLMEmbedding() {
  const queryClient = useQueryClient();
  return useMutation(updateSearchEnginesMutationOptions(queryClient));
}
...
export function useDeleteLLMEmbedding() {
  const queryClient = useQueryClient();
  return useMutation(deleteSearchEnginesMutationOptions(queryClient));
}
