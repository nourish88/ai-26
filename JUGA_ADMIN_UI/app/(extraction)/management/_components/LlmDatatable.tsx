...
import type { ExtractionEngineType } from "../_types/types";
...
type LLMTableRow = ExtractionEngineType | { id: "new" | "update" };
type SwitchStates = { word: boolean; pdf: boolean; txt: boolean };

const INITIAL_NEW_LLM_STATE: Omit<ExtractionEngineType, "id"> = {
  identifier: "",
  word: false,
  txt: false,
  pdf: false,
...
function LLMEmbeddingDatatable() {
  ...
  const validateLLMData = useCallback((data: Omit<ExtractionEngineType, "id">): string => {
  ...
  const handleEditingLLMChange = useCallback(
    (field: keyof ExtractionEngineType, value: string | number) => {
      if (validationError) setValidationError("");
      if (!editingLLMDataRef.current) return;
  ...
  const handleEditLLMModel = useCallback((llm: ExtractionEngineType) => {
    setIsAddingNew(false);
    ...
    () => [
      {
        ...
        header: "Model Name",
        cell: ({ row }) => {
          const llm = row.original as ExtractionEngineType;
          ...
          if (row.original.id === "new") {
            return (
              <div className="flex items-center justify-center">
                ...
                <Switch checked={newSwitchStates.word} onCheckedChange={(checked) => handleNewSwitchChange("word", checked)} />
              </div>
            );
          }
          if (editingRowId === llm.id) {
            return (
              <div className="flex items-center justify-center">
                {/* --- CHANGE: Bind Switch to reactive state --- */}
                <Switch checked={editingSwitchStates?.word || false} onCheckedChange={(checked) => handleEditingSwitchChange("word", checked)} />
              </div>
            );
          }
          return (
            <div className="flex items-center justify-center">
              <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${llm.word ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>{llm.word ? "Enabled" : "Disabled"}</div>
            </div>
          );
        },
      },
      // --- All other Switch columns (pdf, txt) follow the same pattern ---
      {
        ...
        cell: ({ row }) => {
          ...
          if (row.original.id === "new") {
            return (
              <div className="flex items-center justify-center">
                <Switch checked={newSwitchStates.pdf} onCheckedChange={(checked) => handleNewSwitchChange("pdf", checked)} />
              </div>
            );
          }
          if (editingRowId === llm.id) {
            return (
              <div className="flex items-center justify-center">
                <Switch checked={editingSwitchStates?.pdf || false} onCheckedChange={(checked) => handleEditingSwitchChange("pdf", checked)} />
              </div>
            );
          }
          return (
            <div className="flex items-center justify-center">
              <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${llm.pdf ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>{llm.pdf ? "Enabled" : "Disabled"}</div>
            </div>
          );
        },
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (row.original.id === "new") {
            return (
              <div className="flex items-center justify-center">
                <Switch checked={newSwitchStates.txt} onCheckedChange={(checked) => handleNewSwitchChange("txt", checked)} />
              </div>
            );
          }
          if (editingRowId === llm.id) {
            return (
              <div className="flex items-center justify-center">
                <Switch checked={editingSwitchStates?.txt || false} onCheckedChange={(checked) => handleEditingSwitchChange("txt", checked)} />
              </div>
            );
          }
          return (
            <div className="flex items-center justify-center">
              <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${llm.txt ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>{llm.txt ? "Enabled" : "Disabled"}</div>
            </div>
          );
        },
      },
  ...
  const originalData: ExtractionEngineType[] = data?.items || data || [];
