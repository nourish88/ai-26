...
public class HttpContextTokenProvider(IHttpContextAccessor httpContextAccessor) : ITokenProvider
{
    ...
    public async Task<string> GetToken(string authenticationSchema, TokenType type)
    {
        return await(httpContextAccessor.HttpContext == null ? Task.FromResult("") : httpContextAccessor.HttpContext.GetTokenAsync(authenticationSchema, type.ToDescription()));
    }
}